...
it('serves the shapes the react client expects', function () {
    ...
    // Series.js plays episodes through the gated episode stream endpoint; the
    // raw source is never embedded in the catalogue response for buyers.
    $catalogue = $this->getJson("/api/videos/{$series->id}", tokenHeader($user->fresh()))
        ->assertJsonPath('purchased', true);
    expect($catalogue->json('episodes.0.video_url'))->toBeNull();

    $episodeId = $catalogue->json('episodes.0.id');
    $this->postJson("/api/videos/{$series->id}/episodes/{$episodeId}/stream", [], tokenHeader($user->fresh()))
        ->assertOk()
        ->assertJsonPath('data.url', 'https://cdn/e1.mp4');
});
